...
//   let total = 0;
//   total += (parseFloat(scores.st1) || 0) * 0.1;      // 10%
//   total += (parseFloat(scores.ws) || 0) * 0.1;       // 10%
//   total += (parseFloat(scores.mid_exam) || 0) * 0.2;  // 20%
//   total += (parseFloat(scores.project) || 0) * 0.1;   // 10%
//   total += (parseFloat(scores.st2) || 0) * 0.1;       // 10%
//   total += (parseFloat(scores.home_class_work) || 0) * 0.05; // 5%
//   total += (parseFloat(scores.class_activity) || 0) * 0.05; // 5%
//   total += (parseFloat(scores.final_exam) || 0) * 0.3; // 30%
...
//         COALESCE(m.total_score, 
//           (m.st1 * 0.1 + m.ws * 0.1 + m.mid_exam * 0.2 + m.project * 0.1 +
//            m.st2 * 0.1 + m.home_class_work * 0.05 + m.class_activity * 0.05 + m.final_exam * 0.3)
//         ) AS calculated_total
//       FROM marks m
...
//     // Get all student marks data with all weight fields
...
//         total_score: row.total_score || row.calculated_total,
//         term: row.term_name,
//         year: row.year_name
//       });
//     });
...
//     // Get enrollments for teacher's assigned sections/academic-years
...
// // Add mark with all weight fields
...
//         suggestion: "Use the edit feature in admin panel to modify existing marks."
//       });
//     }

//     // 6. Calculate total score
//     const total_score = calculateTotalScore(scores);

//     // 7. Insert the mark with all fields
//     await pool.query(
//       `INSERT INTO marks 
//        (enrollments_id, subjects_id, st1, ws, mid_exam, project, st2, 
//         home_class_work, class_activity, final_exam, total_score)
//        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, 
//       [
//         enrollments_id, subjects_id,
...
// // Update mark with all weight fields
...
//       SELECT m.*, e.sections_id, e.academic_year_id
//       FROM marks m
//       JOIN enrollments e ON m.enrollments_id = e.id
//       WHERE m.id = ?
//     `, [id]);
...
//     // Get all marks for teacher's assigned students with total_score
//     const [marks] = await pool.query(`
//       SELECT
//         e.student_id,
//         s.Sex AS gender,
...
//     const failingStudents = marks.filter(m => m.final_score < 50);
...
//         (marks.reduce((sum, m) => sum + m.final_score, 0) / marks.length).toFixed(2) : 0,
//       passingRate: marks.length > 0 ? 
//         ((marks.filter(m => m.final_score >= 50).length / marks.length) * 100).toFixed(2) : 0,
//       failingStudents: {
